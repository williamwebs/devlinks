...
const Links = () => {
  ...
  // handle drodown options selection
  ...
  // find index of he links
  const getLinkPosition = (id) => profile?.links.findIndex((i) => i.id === id);
  ...
  const handleDragEnd = async (event) => {
    ...
    const originalPos = getLinkPosition(active.id);
    const newPos = getLinkPosition(over.id);

    // Update links array in the component state
    setProfile((prevProfile) => ({
      ...prevProfile,
      links: arrayMove(prevProfile.links, originalPos, newPos),
    }));

    // Update links array in Firestore
    try {
      const linksRef = doc(db, "pages", session.user.email);
      await updateDoc(linksRef, {
        links: arrayMove(profile.links, originalPos, newPos),
      });
